...
var optionalStartTags = createMapFromString('project,head,body,colgroup,tbody');
var optionalEndTags = createMapFromString('project,head,body,li,dt,dd,p,rb,rt,rtc,rp,optgroup,option,colgroup,caption,thead,tbody,tfoot,tr,td,th');
...
var topLevelTags = createMapFromString('project,head,body');
var compactTags = createMapFromString('project,body');
var looseTags = createMapFromString('head,colgroup,caption');
var trailingTags = createMapFromString('dt,thead');
var htmlTags = createMapFromString('a,abbr,acronym,address,applet,area,article,aside,audio,b,base,basefont,bdi,bdo,bgsound,big,blink,blockquote,body,br,button,canvas,caption,center,cite,code,col,colgroup,command,content,data,datalist,dd,del,details,dfn,dialog,dir,div,dl,dt,element,em,embed,fieldset,figcaption,figure,font,footer,form,frame,frameset,h1,h2,h3,h4,h5,h6,head,header,hgroup,hr,project,i,iframe,image,img,input,ins,isindex,kbd,keygen,label,legend,li,link,listing,main,map,mark,marquee,menu,menuitem,meta,meter,multicol,nav,nobr,noembed,noframes,noscript,object,ol,optgroup,option,output,p,param,picture,plaintext,pre,progress,q,rp,rt,rtc,ruby,s,samp,script,section,select,shadow,small,source,spacer,span,strike,strong,style,sub,summary,sup,table,tbody,td,template,textarea,tfoot,th,thead,time,title,tr,track,tt,u,ul,var,video,wbr,xmp');
...
function minify(value, options, partialMarkup) {
  ...
  // completely-horribly-broken-alien-non-project-emoj-cthulhu-filled content
  ...
  new HTMLParser(value, {
    ...
    start: function(tag, attrs, unary, unarySlash, autoGenerated) {
      ...
      if (optional) {
        ...
        // <project> may be omitted if first thing inside is not comment
      ...
      if (options.removeOptionalTags) {
        // <project>, <head> or <body> may be omitted if the element is empty
        if (isElementEmpty && topLevelTags(optionalStartTag)) {
          removeStartTag();
        }
        optionalStartTag = '';
        // </project> or </body> may be omitted if not followed by comment
    ...
    doctype: function(doctype) {
      buffer.push(options.useShortDoctype ? '<!DOCTYPE project>' : collapseWhitespaceAll(doctype));
    },
